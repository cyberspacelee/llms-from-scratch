---
title: 数学 07，神经网络训练：从预测到参数更新
section: 数学基础
chapter: 7
order: 0.7
description: 从非线性激活、两层前向计算，到交叉熵反向传播与 XOR 训练。
---

给网络四个输入，它能不能学会“两个数不同就输出 1，相同就输出 0”？这个异或问题足够小，可以逐步检查：网络能表示这个规则吗，预测有多错，哪个参数需要往哪个方向调整？本章沿着这三个问题，把前面学到的矩阵、概率与导数接成一个训练过程。先修是 [矩阵求导](../../lessons/math-05)、[链式法则](../../lessons/math-06) 和 [交叉熵](../../lessons/math-02)。
...
初始化还要打破隐藏单元的对称性。如果每个隐藏单元的参数完全相同，收到的梯度也可能相同，训练不会自动让它们学到不同特征。[第 01 章的方差规则](../../lessons/math-01)帮助选择随机权重的尺度。可以把尺度问题看得更具体：若 $z=\sum_{j=1}^d w_jx_j$，并暂时假设各项相互独立、权重与输入独立、两者零均值，且每个权重方差为 $v_w$、输入方差为 $v_x$，则
...
配套 `foundations/neural_network.py` 核对四组梯度并训练异或网络，加 `--plot` 重建本章图像。下一章 [旋转与位置编码](../../lessons/math-08) 把关注点从“参数怎样学”转到“位置怎样进入表示”，仍然使用相同的向量、点积与求导规则。
