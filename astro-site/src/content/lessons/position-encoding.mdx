...
完整的生成元与矩阵指数推导已独立到 [数学基础 · 第 07 章](../../lessons/mathematics#section-7)。这里使用其结论：二维旋转可写成 $R(\phi)=e^{\phi J}=\cos\phi I+\sin\phi J$，其中 $J^2=-I$。
...
回到 [数学基础 · 点积与投影](../../lessons/mathematics#section-3) 中的 $q^\top k=\|q\|\|k\|\cos\theta$。第一点洞察是：**固定长度，通过改变 Q/K 的夹角就能调节点积。** 在一个二维子空间中，设内容向量的相位分别为 $\alpha,\beta$，角频率为 $\omega$。位置 $i,j$ 分别添加相位 $i\omega,j\omega$，于是：
