...
chapter: 1
order: 1
description: 从注意力的对称性，到三角函数、矩阵指数与旋转。
---

import DotProductExplorer from "../../components/DotProductExplorer"
import RotationLab from "../../components/DiagramPlaceholder"
import FrequencyLab from "../../components/DiagramPlaceholder"
...
数学准备已独立为 [00 数学基础](../../lessons/mathematics)。如果对点积、范数或欧拉公式不熟悉，可以先阅读对应章节；本篇从注意力的计算与顺序问题开始。

<DotProductExplorer client:visible />
...
<RotationLab title="旋转实验" client:visible />

实验采用标准 $(\cos,\sin)$ 坐标，固定内容 $q=k=(1,0)^\top$，并使用 $\theta=\pi/8$。保持位移不变时改变起点，两向量一起转，夹角与点积不变；这是稍后 RoPE 要利用的性质。

### 进阶阅读：为什么连续、保长的变换导向旋转？

完整的生成元与矩阵指数推导已独立到 [数学 08 · 矩阵指数](../../lessons/math-08#04--矩阵指数连续旋转从哪里来)。这里使用其结论：二维旋转可写成 $R(\phi)=e^{\phi J}=\cos\phi I+\sin\phi J$，其中 $J^2=-I$。
...
<FrequencyLab title="频率实验" client:visible />
...
回到 [数学基础 · 点积与投影](../../lessons/math-03#02--长度与方向范数点积和投影) 中的 $q^\top k=\|q\|\|k\|\cos\theta$。第一点洞察是：**固定长度，通过改变 Q/K 的夹角就能调节点积。** 在一个二维子空间中，设内容向量的相位分别为 $\alpha,\beta$，角频率为 $\omega$。位置 $i,j$ 分别添加相位 $i\omega,j\omega$，于是：
...
训练往往并行处理整段序列；生成时不断追加 token。位置编码规则不因此改变，改变的是张量形状、索引和哪些中间结果可以复用。继续阅读 [推理篇：位置编码与 KV cache](../../lessons/kv-cache)。
