---
title: 数学 08，旋转与位置编码：从保长变换到相对位置
section: 数学基础
chapter: 8
order: 0.8
description: 从正交变换、极坐标和复数，推到多频率旋转与 RoPE。
---

同一个词出现在两个位置，注意力怎样区分它们，又怎样比较它们的相对距离？一种做法是保留表示的长度，按照位置旋转表示。本章先研究这种运算本身，再解释它怎样让两个位置的差进入点积。先读 [03 向量与矩阵](../../lessons/math-03) 的范数、点积和转置；矩阵指数是选读，其余各节构成一条完整路线。角度默认使用弧度，范数均指欧氏范数。
...
这里 `(2,)` 数组的 `@` 表示向量点积，数学推导中的列向量形状在 [03 章](../../lessons/math-03) 有显式版本。需要自动求导与注意力实现时，继续位置编码章节的 PyTorch 数值验证。
...
本章的图解、手算例和教学路线是为讲解组织的内容。接下来阅读 [01 位置编码](../../lessons/position-encoding)，看模型为什么需要这些数学结构，以及怎样把它们应用到 Q/K。
