---
title: 从一卡到一簇
question: 多出来的卡切哪一维，通信走哪条线，状态放在哪台机器上
description: 张量、流水、数据与专家并行各自切开什么、每步多出哪些通信；同一次集合通信的秒数由拓扑决定；再看 prefill/decode 解耦、训练状态分片与端边云的放置。
order: 5
---
...
[S1](/systems/ledger/) 的 16 字节已经让一个 8B 模型超过单卡。两条正交的办法可以叠用。梯度检查点少留激活，反向时重算。ZeRO 按数据并行的成员数 $D$ 切开常驻状态：先切 12 字节的优化器状态（fp32 主权重、动量与方差），再切梯度，最后连参数也切，用到时再集齐。切得越细，单卡常驻越小，集合通信越多。分片的定义见 [ZeRO](https://arxiv.org/abs/1910.02054)。训练系统要同时安排这三样：激活留多少、优化器状态放在谁那里、微批怎样填满流水。这是 [AI Infra Book 第 10 章](https://bojieli.github.io/ai-infra-book/manuscripts/10-%E8%AE%AD%E7%BB%83%E7%B3%BB%E7%BB%9F.html) 的问题；分布式推理一侧的放置、扩容和恢复见 [第 9 章](https://bojieli.github.io/ai-infra-book/manuscripts/09-%E5%88%86%E5%B8%83%E5%BC%8F%E6%8E%A8%E7%90%86.html)。
...
<details>
<summary>1. 流水并行分 4 个阶段、同时有 12 个微批在飞，气泡大约占多少？要把它压到 5% 以下，微批至少要多少？</summary>

气泡比例约 $(P-1)/M=3/12=25\%$。要求 $3/M<0.05$，得 $M>60$，至少 61 个微批。微批越多，每个越小，单次矩阵乘的算术强度越低；这就是交错阶段等排程要解决的矛盾。

</details>

<details>
<summary>2. ZeRO 把优化器状态切到 8 张数据并行卡上，8B 模型每卡常驻的 16 字节变成多少？</summary>

16 字节里，fp32 主权重、动量与方差共 12 字节属于优化器状态，切 8 份后每卡 1.5 字节；低精度参数和梯度各 2 字节仍完整保留。每个参数约 $2+2+1.5=5.5$ 字节，8B 模型约 $8.03\times10^9\times5.5\approx44$ GB，还没算激活。再切梯度和参数，常驻继续下降，代价是更多的集合通信。

</details>

<details>
<summary>3. 为什么“把张量切到另一张卡”不是免费的容量？</summary>

切开后每一层都要做集合通信，字节数由切分方式决定，秒数由链路决定。按产品页数字，SXM 的 NVLink 900 GB/s 小于本卡 HBM 的 3.35 TB/s；穿过网卡的链路还要再低一个数量级以上。容量换到了，时间要从通信里付。
