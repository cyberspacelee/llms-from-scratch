...
数学准备已独立为 [00 数学基础](../foundations/mathematics.html)。如果对点积、范数或欧拉公式不熟悉，可以先阅读对应章节；本篇从注意力的计算与顺序问题开始。
...
原有内容相位差与相对位置相位差共同决定分数。固定其他候选的 logit 时，提高这一项会提高对应 softmax 权重；若多项同时变化，则需比较整行分数。还要区分“夹角增大”与“位置距离增大”：相位具有周期性，所以距离增加不保证点积单调下降。

第二点洞察是：**标准 RoPE 的旋转严格保持 Q/K 范数，因此不会在这一步抹去长度中可能已有的内容信息。** 这里的“可能”很关键：范数是学习所得表示的一个自由度，可能与语义或注意力打分尺度有关；这是解释性假设，不能由保范数定理直接推出“范数就是语义”。Q/K 已经过学习投影，后续层的输入还融合了上下文，不能将它们的范数等同于原始 token embedding 的范数。

例如 $(1,0)^\top$ 与 $(0,1)^\top$ 范数相同，但与 $k=(1,0)^\top$ 的点积分别是 1 和 0，说明内容匹配也依赖方向。更准确的说法是：RoPE 保留旋转前的长度，用位置相关旋转调制已有的内容方向；给定位置时旋转可逆，但“范数不变”本身不等于证明语义效果不变。

高维 RoPE 的各二维分块拥有不同频率。若第 $m$ 对的范数为 $r_{q,m},r_{k,m}$、内容相位为 $\alpha_m,\beta_m$，则：

$$
\widetilde q_i^\top\widetilde k_j
=\sum_{m=0}^{d_h/2-1}r_{q,m}r_{k,m}
\cos\big(\beta_m-\alpha_m+(j-i)\omega_m\big).
$$

零向量分块的贡献为零，无需定义其相位。每块范数及总范数都保持不变；高维整体仍满足“总范数乘积 × 整体夹角余弦”，但一般不能把整体夹角写成一个统一频率的线性位置偏移。

上述几何展开是本讲义补充的推导；RoPE 的二维复数构造、高维分块和正交性质可对照 [RoFormer，§3.2–3.4](https://arxiv.org/html/2104.09864v5#S3)。该论文提供旋转结构的依据，并未证明 token 语义只由范数编码。

### 一个完整算例：长度不变，分数从正变负

为方便手算，只考虑一个二维子空间，令 $q=(2,0)^\top$、$k=(\sqrt3,1)^\top$。两者范数都为 2，内容相位分别为 0 与 $\pi/6$，因此原点积为 $4\cos(\pi/6)=2\sqrt3$。选取角频率 $\omega=\pi/6$；这是讲解用频率，不是标准频率表的数值。
...
用相位法重算：新相位差为 $\pi/6+(3-1)\pi/6=\pi/2$，所以点积为 $4\cos(\pi/2)=0$。两个范数仍然都是 2，分数变化全部来自相对方向。

继续固定这两个内容向量，只改变位移 $\Delta=j-i$：
...
还要区分两种“保留”。**数值不变**指范数相同；**信息可恢复**指给定位置后可以使用 $q=T_p^\top\widetilde q$ 恢复整个向量。后者依赖已知位置和旋转规则；如果位置未知，不能从旋转结果唯一拆出原内容方向和位置相位。两者都不等于“后续网络的所有行为不变”，因为改变点积正是引入位置的目的。

在高维里，只看总范数还会漏掉每个频率分块的幅度分配。比如四维向量 $q_A=(1,0,0,0)^\top$ 与 $q_B=(0,0,1,0)^\top$ 总范数都为 1，但前者全部落在第一对，后者全部落在第二对。面对固定 $k=(1,0,1,0)^\top$，原点积都为 1，位置旋转后的分数却分别为 $\cos(\Delta\omega_0)$ 和 $\cos(\Delta\omega_1)$。总范数相同，并不意味着对位置的响应相同。
...
标准 RoPE 本身不会把非零 Q/K 自动归一化。若模型另外使用 Q/K 归一化、额外缩放或非正交位置变换，应按实际算子重新分析；本节的保长结论只针对纯旋转。也不要把 hidden state 的归一化直接等同于 Q/K 单位化，后续学习投影可能再次改变长度。
